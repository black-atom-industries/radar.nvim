...
describe("locks", function()
    ...
    describe("get_next_unused_label", function()
        ...
        it("returns labels in order defined in config", function()
            ...
            for i = 1, 5 do
                table.insert(state.locks, { label = tostring(i), filename = "file" .. i .. ".lua" })
            end
        ...
        it("throws error when all slots are used", function()
            ...
            for _, label in ipairs(test_config.keys.locks) do
                table.insert(state.locks, { label = label, filename = "file_" .. label .. ".lua" })
            end
    ...
    describe("toggle", function()
        ...
        it("adds lock when file not locked", function()
            local lock = locks.toggle("new.lua", test_config, mock_persistence, mock_mini_radar)
        ...
        it("removes lock when file is locked", function()
            ...
            local removed_lock = locks.toggle("existing.lua", test_config, mock_persistence, mock_mini_radar)
    ...
    describe("lock_current_buffer", function()
        ...
        before_each(function()
            ...
            vim.api.nvim_buf_get_name = function() return "/test/path/file.lua" end
        end)
        ...
        it("uses current buffer when buf_nr is nil", function()
            ...
            MiniTest.expect.equality(state.locks[1].filename, vim.fn.fnamemodify("/test/path/file.lua", test_config.path_format))
        end)

        it("creates mini radar when it doesn't exist", function()
            local create_called = false
            mock_mini_radar.create = function() create_called = true end
        ...
        it("updates mini radar when it exists", function()
            ...
            mock_mini_radar.exists = function() return true end
            mock_mini_radar.update = function() update_called = true end
