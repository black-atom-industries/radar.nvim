    ...
    local function setup_test_state()
        ...
        mock_mini_radar = {
            get_formatted_filepath = function(path, config)
                return vim.fn.fnamemodify(path, config.path_format or ":p:.")
            end,
            update = function() end
        }
        ...
        for _, func in ipairs({
            "nvim_create_buf", "nvim_buf_set_lines", "nvim_buf_get_lines",
            "nvim_set_option_value", "nvim_buf_set_name", "nvim_open_win",
            "nvim_win_close", "nvim_win_is_valid", "nvim_create_augroup",
            "nvim_create_autocmd", "nvim_buf_set_keymap", "nvim_win_get_cursor"
        }) do
