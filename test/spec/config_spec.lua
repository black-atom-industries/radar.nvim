...
describe("config", function()
    describe("default configuration", function()
        it("has all required keys", function()
            ...
            MiniTest.expect.equality(type(config.default.width), "number")
            MiniTest.expect.equality(type(config.default.max_session_files), "number")
            MiniTest.expect.equality(type(config.default.persist_path), "string")
        end)
        ...
        it("has valid UI configuration", function()
            ...
            -- Check UI settings
            MiniTest.expect.equality(type(cfg.width), "number")
            MiniTest.expect.equality(type(cfg.winblend), "number")
            MiniTest.expect.equality(type(cfg.path_format), "string")
            MiniTest.expect.equality(type(cfg.show_empty_message), "boolean")

            -- Check headers
            MiniTest.expect.equality(type(cfg.locks_header), "string")
            MiniTest.expect.equality(type(cfg.recent_header), "string")
            MiniTest.expect.equality(type(cfg.title), "string")

            -- Check edit config
            MiniTest.expect.equality(type(cfg.edit_width_padding), "number")
            MiniTest.expect.equality(type(cfg.edit_max_height), "number")
            MiniTest.expect.equality(type(cfg.edit_min_width), "number")
        end)

        it("has valid behavior configuration", function()
            local cfg = config.default

            MiniTest.expect.equality(type(cfg.max_session_files), "number")
            MiniTest.expect.equality(type(cfg.defer_persist_ms), "number")
            MiniTest.expect.equality(type(cfg.hide_on_collision), "boolean")
            MiniTest.expect.equality(type(cfg.collision_padding), "number")
        end)

        it("has valid persistence configuration", function()
            local cfg = config.default

            MiniTest.expect.equality(type(cfg.persist_path), "string")

            -- Check that path is reasonable
            MiniTest.expect.equality(vim.startswith(cfg.persist_path, "/"), true)
            MiniTest.expect.equality(cfg.persist_path:match("%.json$") ~= nil, true)
        end)
