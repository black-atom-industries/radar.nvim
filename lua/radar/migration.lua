...
function M.migrate_v0_to_v1(data)
    ...
    for project_path, branches in pairs(data) do
        ...
        for branch_name, branch_data in pairs(branches) do
            ...
            migrated.projects[project_path][branch_name] = M.ensure_last_accessed(branch_data)
        end
    end
...
function M.migrate(data)
    ...
    vim.notify(
        string.format("Unknown radar data version: %s. Using data as-is.", tostring(version)),
        vim.log.levels.WARN
    )
    return data
end
